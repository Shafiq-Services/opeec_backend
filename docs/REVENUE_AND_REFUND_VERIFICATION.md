...
2. **On order completion (Finished):**
   - **Seller payout:** `triggerAutomaticPayout(orderId)` transfers **rental_fee − stripe_fee − max(platform_fee, 0.70) + (penalty when penalty_apply)** to the seller’s Stripe Connect account. Owner pays Stripe fee and platform fee (min $0.70).
...
3. **Code references:**
   - `controllers/stripeConnectController.js`: `triggerAutomaticPayout` — transfer = `rental_fee - stripe_fee - max(platform_fee, 0.70) + (penalty when penalty_apply)`.
